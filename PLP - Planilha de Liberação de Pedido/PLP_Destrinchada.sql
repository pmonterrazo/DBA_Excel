SELECT DISTINCT A.VDPEDCPE_CODCLI  AS 'COD. CLIENTE', CAST(SUBSTRING(CAST(A.VDPEDCPE_NPED AS VARCHAR(300)),9,5) AS VARCHAR(300)) AS 'PEDIDO', B.VDCLICLI_SIGLA AS 'FANTASIA', A.VDPEDCPE_VEN_ORI_CLI AS 'VD', A.VDPEDCPE_CODSUP AS 'SUP',

CASE

      WHEN(A.VDPEDCPE_TPCOBR)  >= 4 THEN 'PRAZO'
      WHEN(A.VDPEDCPE_TPCOBR)  = 1 THEN 'A VISTA' END AS 'PRAZO',

CAST(A.VDPEDCPE_CPG AS VARCHAR(300)) || ' - ' || CAST(E.VDCADPAG_DESCR AS VARCHAR(300)) AS 'COND. PG',

CASE

    WHEN F.VDPEDIPE_OCOKD = 1 THEN 'VENDA' 
    WHEN F.VDPEDIPE_OCOKD = 2 THEN 'BONIFICAÇÃO'
    WHEN F.VDPEDIPE_OCOKD = 3 THEN 'TROCA' 
...
INNER JOIN PEDIT0 AS F ON A.VDPEDCPE_NPED=F.VDPEDIPE_NIT
